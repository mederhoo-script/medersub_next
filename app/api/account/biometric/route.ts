...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'purchase-options') {
      if (!credentials?.length) return NextResponse.json({ error: 'Set up fingerprint or Face ID in Account Settings first.' }, { status: 400 });
    ...
    if (!challenge || new Date(challenge.expires_at) < new Date()) return NextResponse.json({ error: 'Biometric request expired. Try again.' }, { status: 400 });
    ...
    if (action === 'enroll-verify' && challenge.purpose === 'enroll') {
      ...
      if (!verified.verified || !verified.registrationInfo) return NextResponse.json({ error: 'Biometric registration could not be verified.' }, { status: 400 });
    ...
    if (action === 'purchase-verify' && challenge.purpose === 'purchase') {
      ...
      if (!credential) return NextResponse.json({ error: 'Unknown biometric credential.' }, { status: 400 });
      ...
      if (!verified.verified) return NextResponse.json({ error: 'Biometric approval could not be verified.' }, { status: 400 });
    ...
    return NextResponse.json({ error: 'Invalid biometric request.' }, { status: 400 });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Biometric request failed.' }, { status: 500 });
  }
}
